...
class mainWindow(QtWidgets.QMainWindow, uic.loadUiType(os.path.join(os.path.dirname(__file__), '_lib/_main.ui'))[0]):
    def __init__(self):
        ...
        self.action_OpenESG.triggered.connect(self.openESG)
    ...
    def updateFiles(self):
        ...
        if files[0].endswith('.prf'):
            ...
            self.openXYE(files)
        elif files[0].endswith('.fit'):
            file = ['{}.esg'.format(files[0].split('(')[0])]
            self.openESG(file)
        return
    ...
    def updatePatternPlot(self):
        ...
        self.parw.updateVline(len(self.files)-h_val+0.5)
    ...
    def updateHLine(self):
        val = len(self.files)-self.parw.vline.value()
            ...
            for line in f:
                if '_pd_meas_intensity_total' in line:
                    for line in f:
                        ...
                        tth[i].append(np.arctan(x/dist)*180/np.pi)
                        I[i].append(y)
                    tth.append([])
                    I.append([])
                    i += 1
        tth.sort(key=lambda tth: len(tth))
        tth = tth[-1]
        if tth[0]>tth[1]:
            tth.reverse()
            [x.reverse() for x in I]
            
        length = max(map(len,I))           
        tth = np.array(tth,float)
        I = np.array([x+[None]*(length-len(x)) for x in I[:-1]],dtype=float) #shape: (bins, datapoints)
        return tth, I, dist
    
    def openESG(self, file=None):
        if not isinstance(file,list):
            path = ''
            if not path:
                path = QtCore.QDir.currentPath() 
            file, _ =  QtWidgets.QFileDialog.getOpenFileName(self, 'Select .esg file', path , '*.esg')
        else:
            file = file[0]
            path = os.path.dirname(file)
        if len(file)<1:
            return
        if file.endswith('.esg'):
            try:
                tth = []
                rp = []
                sub_plots = {}
                ...
                #####  Open the primary .esg file  #####
                tth_full, yobs, dist = self.readEsg(file)
               
                #####  Open subsequent .fit files  #####
                files = ['{}({}).fit'.format(file[:-4],i) for i in range(yobs.shape[0])]
                for f in files:
                    if not os.path.exists(f):
                        QtWidgets.QMessageBox.warning(self,'User input required','Unable to locate .fit files!\nPlease locate the {} files manually'.format(yobs.shape[0]))
                        files, _ =  QtWidgets.QFileDialog.getOpenFileNames(self, 'Select {} .fit files for {}'.format(yobs.shape[0],file), path , '*.fit')
                        if len(files) != yobs.shape[0]:
                            raise
                        break
                progress = self.progressWindow("Reading files", "Abort", 0, len(files),'Refinement Evaluator',QtGui.QIcon(":icons/Main.png"))             
                ...
                for i, f in enumerate(files):
                    progress.setValue(i)
                    data = self.readFit(f,dist)
                    tth.append(data.pop('tth'))
                    # Find matching 2theta values between observed and calculated 2theta
                    tth_matching = np.array([(x in tth[i]) for x in tth_full])
                    # Make all intensity arrays same length
                    I = np.full(yobs.shape[1],0.0, dtype=float)
                    I[np.isnan(yobs[i,:])]=np.nan
                    I[tth_matching] = data.pop('I')
                    ycal = I
                    res = yobs[i,:]-ycal
                    rp.append(np.nansum(np.abs(res))/np.nansum(yobs[i,:])*100)
                    bgr = data.pop('background')
                    im[0].append(yobs[i,:])
                    im[1].append(ycal)
                    im[2].append(res)                 
                    ...
                    for key in data:
                        # Make all intensity arrays same length
                        y = np.full(yobs.shape[1],0.0, dtype=float)
                        y[tth_matching] = data[key]+bgr
                        try:
                ...
                """
                # Find longest array of 2theta
                tth.sort(key=lambda tth: len(tth))
                tth = tth[-1]
                
                tth = tth_full.copy()
                tth[~tth_matching] = np.nan
                
                # Make all intensity arrays same length
                length = max(map(len,ycal))
                for i, x in enumerate(ycal):
                    for k in range(length-x.shape[0]):
                        ycal[i] = np.append(x,np.nan)
                ycal = np.array(ycal)
                """
                ...
                self.sub_plots = {key:np.array(sub_plots[key]) for key in sub_plots}
                progress.setValue(len(files))
                self.setMultiImages(tth_full,im)
                for key in sub_plots:
                    self.ppw.addSubplot(key=key)
                self.updatePatternPlot()
                self.updateParameterPlot()
                ...
                QtWidgets.QMessageBox.warning(self,'Warning','Unable to open files!')
        else:
            QtWidgets.QMessageBox.warning(self,'Warning','Unable to open .esg file!')             
        
    def readFit(self,fname,dist):
        """Read *.fit files from MAUD. 2theta values are calculated from provided detector distance. Return dictionary with tth, I, and additional columns."""
        keys = ['tth','I']
        with open(fname,'r') as f:
            # Read header
            for line in f:
            ...
            for line in f:
                ...
                if not '_' in line:
                    ...
                    break
            data = np.append(data,np.loadtxt(f, dtype=float),axis=0)
            if data[0,0]>data[1,0]:
                data = np.flipud(data)
        # Convert from detector coordinates to 2theta
        data[:,0] = np.arctan(data[:,0]/dist)*180/np.pi
